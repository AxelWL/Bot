 /*
* Bot se creo para fines educativos
* bot se reserva todos los derechos de autor
* © 2022 BOT, SAC.
...
const { Axeltera } = require('./src/Axeltera')
...
const vcard = 'BEGIN:VCARD\n' // Tarjeta de contacto
            ...
            + 'FN:Axel\n' // Nombre
            + 'ORG:Axel;\n' // Propietario
            + 'TEL;type=CELL;type=VOICE;waid=51927769540:+51 927 769 540\n' // ID de WhatsApp + número de teléfono
            + 'END:VCARD'
...
/******FIN DE ARCHIVOS ANTILINK POR AXEL******/
...
function addMetadata(packname, author) {	
	if (!packname) packname = 'AxelBot'; if (!author) author = 'Axel';	
		...
		try {
			...
			mess = {
				...
				only: {
					...
					ownerB: '[❗] Este comando solo lo usa AxelBot',
...
/******ENTRADA FIN DE FUNCIONES******/
		...
		break
		case 'Axeltera':
		client.sendMessage(from, Axeltera(prefix, sender), text, {quoted: mek})
		break
...
/******JUEGOS Axel LA PUTA MADRE NO TE OLVIDES******/
		...
		if (!isUser) return reply(mess.only.daftarB)
			...
			case 'todos':
				client.updatePresence(from, Presence.composing) 
					...
					mentions('╔══✪〘 *TODOS* 〙✪══\n╠➥'+teks+'╚═〘 _*by Axel*_ 〙', members_id, true)
					break
                                case 'send':
					var pc = body.slice(6)
			...
			//ANTILINKS DE REDES SOCIALES FLACO ACEPTALO SOLO LO ESTAS EDITANDO REALMENTE Axel TE HIZO TODO ESTO	
				...
				//FIN DE ANTILINK HECHO POR Axel
	...
	//SERVICIO DE MUSICA Y VIDEO 			
		...
		if (!isUser) return reply(mess.only.daftarB)
	        reply(mess.only.musica2)
                play = body.slice(5)
                anu = await fetchJson(`https://api.zeks.me/api/ytplaymp3?q=${play}&apikey=24Axel`)
                if (anu.error) return reply(anu.error)
                infomp3 = `*⌜Cancion Encontrada ✅⌟*\n◉ *Título:* ${anu.result.title}\n◉ *Fuente:* ${anu.result.source}\n◉ *Tamaño:* ${anu.result.size}\n\n*ESPERE ENVIANDO SU ARCHIVO MP3 ⚠*\n\n_*Servicio proveido por Axel*_`
                buffer = await getBuffer(anu.result.thumbnail)
                lagu = await getBuffer(anu.result.url_audio)
...
//FIN DE REGISTRO  
	...
	if (budy.startsWith(`La toca 7w7`)) {
        const none = fs.readFileSync('./aniAxel/anime5.mp3');
		client.sendMessage(from, none, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
                  }
	if (budy.startsWith(`Quien es tu sempai botsito`)) {
        const none = fs.readFileSync('./aniAxel/anime4.mp3');
		client.sendMessage(from, none, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
                  }
	if (budy.startsWith(`Me gimes 7u7`)) {
        const none = fs.readFileSync('./aniAxel/anime3.mp3');
		client.sendMessage(from, none, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
                  }
	if (budy.startsWith(`Te amo botsito uwu`)) {
        const none = fs.readFileSync('./aniAxel/anime2.mp3');
		client.sendMessage(from, none, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
                  }
	if (budy.startsWith(`Onichan`)) {
        const none = fs.readFileSync('./aniAxel/anime1.mp3');
		client.sendMessage(from, none, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
                  }
